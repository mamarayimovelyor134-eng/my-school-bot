...
# Wrap reg_finish in a try-except to catch hidden errors
...
async def reg_finish(m: types.Message, state: FSMContext):
    try:
        ...
        await m.answer(TEXTS[lang]["reg_success"], reply_markup=await mkb(uid))
        if role == "driver":
        ...
        await m.answer("тЭМ Ro'yxatdan o'tishda xatolik yuz berdi. Iltimos qayta /start bosing.")
"""
...
print("SUCCESS: reg_finish protected with try-except.")
