...
for line in content:
    if '@dp.message(Command("start"))' in line:
        ...
        # Fix: add logic for existing user in the else block
        new_lines.append('        await m.answer(t(u[6], "welcome"), parse_mode="Markdown", reply_markup=await mkb(m.from_user.id))\n')
        skip = True
        continue
    
    if skip and 'async def' in line and (('set_lang' in line) or ('start_cmd' not in line)):
        skip = False
...
print("SUCCESS: Indentation and logic error fixed.")
