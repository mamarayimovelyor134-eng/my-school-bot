...
ADMIN_ID = 6363231317  # O'z Telegram ID ingizni bu yerga yozing
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=30)) as resp:
                if resp.status == 200:
                    ...
                    return "❌ AI javob bermadi. Internet yoki API muammosi bo'lishi mumkin."
    except Exception as e:
...
def main_menu():
    ...
    builder.row(types.KeyboardButton(text="📊 BSB (Nazorat)"), types.KeyboardButton(text="🎥 Video darslar"))
    builder.row(types.KeyboardButton(text="📝 Onlayn testlar"))
...
async def online_tests(message: types.Message):
    ...
    await message.answer(
        ...
        "O'z bilimingizni reklamalardan xoli va ishonchli portallarda sinab ko'ring. "
        "Faqat rasmiy manbalar tanlab olindi 👇",
        parse_mode="Markdown",
        reply_markup=builder.as_markup()
    )
...
async def keep_alive_loop():
    """Bot uxlab qolmasligi uchun har 5 daqiqada o'zini o'zi 'ping' qiladi."""
    if not WEBHOOK_HOST:
        logging.warning("WEBHOOK_HOST (RENDER_EXTERNAL_URL) topilmadi. O'zini o'zi uyg'otish (Self-ping) ishlamaydi.")
        return

    await asyncio.sleep(60) # Birinchi pingni 1 daqiqadan so'ng yuboramiz
    while True:
        try:
            async with aiohttp.ClientSession() as session:
                # O'z manzilimizga (host) oddiy 'GET' so'rovi yuboramiz
                async with session.get(WEBHOOK_HOST, timeout=10) as resp:
                    logging.info(f"Self-ping yuborildi: {resp.status}")
        except Exception as e:
            logging.error(f"Self-ping xatoligi: {e}")
        
        await asyncio.sleep(300) # Har 5 daqiqa (300 soniya) kutish

async def web_index_handler(request):
    """Render uchun asosiy sahifa (Health check)"""
...
async def on_startup(bot: Bot):
    ...
    # O'zini o'zi uyg'otish (Self-ping) tizimini ishga tushirish
    ...
    if WEBHOOK_HOST:
        ...
        logging.warning("WEBHOOK_HOST topilmadi, polling ishga tushishi mumkin (lekin bu serverda xatoga olib keladi)")
...
def main():
    ...
    # Asosiy sahifa (ping uchun)
    app.router.add_get("/", web_index_handler)
    
    # Aiogram xabarlari uchun handler
